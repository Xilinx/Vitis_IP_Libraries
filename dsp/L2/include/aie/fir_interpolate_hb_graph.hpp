...
class fir_interpolate_hb_graph : public graph {
   private:
        ...
        if
            constexpr(TP_PARA_INTERP_POLY == 1) {
                ...
                lastSSRKernelHBAsym::create_connections(m_firKernels, &in[0], in2, &out[0], out2, coeff, net, net2,
                                                        casc_in, "fir_interpolate_hb_asym.cpp", useRtpPerKernel);
#endif
            }
        else {
            // rearrange taps
            lastSSRKernelSrAsym::create_connections(m_firKernels, &in[0], in2, &out[0], out2, coeff, net, net2, casc_in,
                                                    "fir_sr_asym.cpp", useRtpPerKernel);
            ...
            for (int i = 0; i < TP_SSR; i++) {
                ...
                if
                    constexpr(TP_USE_COEFF_RELOAD == 1) {
                        ...
                        // Map coeffCT, i.e. high polyphase coeff, after the low polyphase coeffs
    ...
    static constexpr unsigned int useAsymImpl = (TP_PARA_INTERP_POLY != 1);
   ...
   public:
    ...
    static constexpr unsigned int rtpPortNumber =
        (TP_SSR * TP_SSR * TP_CASC_LEN) + (TP_PARA_INTERP_POLY > 1 ? TP_SSR : 0);
    ...
    fir_interpolate_hb_graph(const std::vector<TT_COEFF>& taps) {
        ...
        if
            ...
            lastct_kernels::create_and_recurse(m_ct_firKernels,
                                               ct_vector); // fail to configure RTP port on non-existing kernels
        }
        connect_with_ssr();
    }
