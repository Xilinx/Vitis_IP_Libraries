 ...
 */
#ifndef _DSPLIB_FIR_GRAPH_UTILS_SSR_KERNELS_CLASS_HPP_
#define _DSPLIB_FIR_GRAPH_UTILS_SSR_KERNELS_CLASS_HPP_
/*
...
// RTP connection mode constants.
// useRtpPerKernel: RTP port per kernel (used by converted IP).
// useRtpPerSSRPath: RTP port per SSR path (default for ssr_kernels::create_connections).
static constexpr unsigned int useRtpPerKernel = 1;
static constexpr unsigned int useRtpPerSSRPath = 0;
...
class ssr_kernels {
   public:
    ...
    static void create_and_recurse(kernel (&firKernels)[totalKernels], const std::vector<TT_COEFF>& taps) {
        ...
        last_casc_kernels::create_and_recurse(firKernels + kernelStartingIndexInt, ssrPhaseTaps);
    ...
    static void create_and_recurse(kernel (&firKernels)[totalKernels]) {
        ...
        unsigned int kernelStartingIndexInt = isSSRaVector == 1 ? ssrDataPhase * TP_CASC_LEN : kernelStartingIndex;
        last_casc_kernels::create_and_recurse(firKernels + kernelStartingIndexInt);

        if
            constexpr(ssr_params::Bdim > 0) { next_ssr_kernels::create_and_recurse(firKernels); }
    }

    // static unsigned int getRtpPosition(int rtpPortNumber, unsigned int rtpPerKernel = 0) {
    ...
    //     unsigned int ssrInnerPhase = (rtpPortNumber % TP_SSR);
    //     unsigned int cascIndex = rtpPortNumber % TP_CASC_LEN;
    //     if (rtpPerKernel == 0) {
    //         return RTP_PORT_POS;
    //     } else {
    //         return (cascIndex == 0 && ssrInnerPhase == 0) ? CASC_IN_PORT_POS : CASC_IN_PORT_POS + 1;
    //     }
    // }
    // static unsigned int getTDMRtpPosition(int rtpPortNumber) {
    //     // RTP position is after the buffer and cascade input ports.
    //     unsigned int cascIndex = rtpPortNumber % TP_CASC_LEN;
    //     return (cascIndex == 0 ? 1 : 2);
    // }

    template <typename in2_type = port_conditional_array<input, (TP_DUAL_IP == 1), TP_SSR>,
              typename out2_type = port_conditional_array<output, (TP_NUM_OUTPUTS == 2), TP_SSR>,
              typename coeff_type = port_conditional_array<input, (TP_USE_COEFF_RELOAD == 1), TP_SSR> >
            ...
            for (unsigned int ssrInnerPhase = 0; ssrInnerPhase < TP_SSR; ssrInnerPhase++) {
                ...
                if
                    constexpr(TP_USE_COEFF_RELOAD == USE_COEFF_RELOAD_TRUE && TP_CASC_IN == CASC_IN_FALSE) {
                        ...
                        if (rtpPerKernel == useRtpPerSSRPath) {
                            if (ssrInnerPhase == 0) {
                ...
                if
                    constexpr(TP_USE_COEFF_RELOAD == USE_COEFF_RELOAD_TRUE) {
                        if (rtpPerKernel == useRtpPerKernel) {
                            for (int cascIndex = 0; cascIndex < TP_CASC_LEN; cascIndex++) {
                                // RTP position is after the buffer and cascade input ports.
                                // Cascade is present for first kernel in cascade chain, if part of a bigger SSR
                                // structure.
                                unsigned int rtpPosition =
                                    (cascIndex == 0 && ssrInnerPhase == 0 && ssr_params::BTP_CASC_IN == CASC_IN_FALSE)
                                        ? CASC_IN_PORT_POS
                                        : CASC_IN_PORT_POS + 1; // Start of cascade
                                unsigned int kernelIndex = kernelStartingIndex + cascIndex;
    ...
    static void create_tdm_connections(kernel* m_firKernels,
        ...
        for (unsigned int ssrInnerPhase = 0; ssrInnerPhase < TP_SSR; ssrInnerPhase++) {
            ...
            if
                constexpr(TP_USE_COEFF_RELOAD == USE_COEFF_RELOAD_TRUE) {
                    for (int cascIndex = 0; cascIndex < TP_CASC_LEN; cascIndex++) {
                        unsigned int kernelIndex = ssrDataPhase * TP_CASC_LEN + cascIndex;
                        unsigned int rtpPosition = cascIndex == 0 ? 1 : 2;
...
#define COEFF_DATA_ALIGNMENT 1 // data
   ...
   public:
    using last_casc_kernels = casc_kernels<last_casc_params, fir_type>;

    /**
     * @brief Separate TDM FIR taps into a specific cascade segment.
     */
    static std::vector<typename ssr_params::BTT_COEFF> segment_tdm_taps_array_for_cascade(
        ...
        std::vector<TT_COEFF> kernelTaps =
            last_casc_kernels::segment_tdm_taps_array_for_cascade(taps, kernelPosition, cascLen);
        return kernelTaps;
    }
    /**
     * @brief Separate FIR taps into a specific cascade segment.
     */
    static std::vector<typename ssr_params::BTT_COEFF> segment_taps_array_for_cascade(
        const std::vector<typename ssr_params::BTT_COEFF>& taps,
        unsigned int kernelPosition,
        unsigned int cascLen,
        unsigned int isSym = 0) {
        std::vector<TT_COEFF> kernelTaps =
            last_casc_kernels::segment_taps_array_for_cascade(taps, kernelPosition, cascLen, isSym);
        return kernelTaps;
    }
    /**
    ...
    static int getKernelFirRangeLen(int pos) {
        // call casc_kernels class method:
        return last_casc_kernels::getKernelFirRangeLen(pos);
    }
...
#endif //_DSPLIB_FIR_GRAPH_UTILS_SSR_KERNELS_CLASS_HPP_
