 ...
 *throughput.
 *         Currently, only TP_PARA_DECI_POLY=2 is supported for the halfband interpolators with SSR>1. SSR = 1
 *supports TP_PARA_DECI_POLY=1 or 2.
...
class fir_decimate_hb_graph : public graph {
   private:
    ...
    static constexpr unsigned int CASC_IN_PORT_POS = (TP_DUAL_IP == DUAL_IP_DUAL && TP_API == 0) ? 2 : 1;
    ...
    void create_connections() {
        ...
        // if
        //     constexpr(TP_USE_COEFF_RELOAD == 1) { connect<parameter>(coeff[0],
        //     async(m_firKernels[0].in[rtpPortPos])); }
        if
            constexpr(TP_USE_COEFF_RELOAD == 1) {
                unsigned int kernelStartingIndex = 0; // This method is only used in non-SSR context.
                unsigned int ssrCoeffPhase = 0;       // This method is only used in non-SSR context.

                for (int i = 0; i < TP_CASC_LEN; i++) {
                    // RTP position is after the buffer and cascade input ports.
                    // First kernel in chain may have 1 or 2 input buffer ports, remaining kernels always have 1
                    // broadcast input and one cascade.
                    // Hence, RTP always gets index 2.
                    unsigned int rtpPosition = (i == 0) ? CASC_IN_PORT_POS : 2; // Start of cascade
                    unsigned int kernelIndex = kernelStartingIndex + i;
                    kernel* kernelInCascadeChain = m_firKernels + kernelIndex;

                    connect<parameter>(coeff[kernelIndex], async(m_firKernels[kernelIndex].in[rtpPosition]));
                }
            }
...
#else
            // This is not used. If HAS_SYM_PREADD then ssr_kernels class is used and its create_connections method.
...
// Only SSR==1
                ...
                create_connections();
#else
                lastSSRKernelHBAsym::create_connections(m_firKernels, &in[0], in2, &out[0], out2, coeff, net, net2,
                                                        casc_in, "fir_decimate_hb_asym.cpp", useRtpPerKernel);
#endif
            }
        else {
            // 2 polyphases. Split data for asym FIR polyphase and center tap polyphase.
            for (int i = 0; i < TP_SSR; i++) {
                connect<stream>(in[2 * i], in_dummy[i]);
                if
                    constexpr(TP_DUAL_IP == 1) { connect<stream>(in2[2 * i], in2_dummy[i]); }
            }
            lastSSRKernelSrAsym::create_connections(m_firKernels, &in_dummy[0], in2_dummy, &out[0], out2, coeff, net,
                                                    net2, casc_in, "fir_sr_asym.cpp", useRtpPerKernel);
            ...
            for (int i = 0; i < TP_SSR; i++) {
                ...
                if
                    constexpr(TP_USE_COEFF_RELOAD == 1) {
                        ...
                        // Map center tap kernel ports, i.e. high polyphase coeff, after the low polyphase coeffs
                        connect<parameter>(coeff[(TP_SSR * TP_SSR * TP_CASC_LEN) + i],
                                           async(m_ct_firKernels[i].in[RTP_PORT_POS]));
                    }
            }
        }
    }
    ...
    using lastSSRKernelHB = ssrKernelLookup<(TP_SSR * TP_SSR) - 1>;
    using lastSSRKernelCT = ssr_kernels<ct_fir_params<TP_SSR - 1>, sr_asym::fir_sr_asym_tl>;
    ...
    using lastSSRKernelHBAsym = aiemlssrKernelLookup<(TP_SSR * TP_SSR) - 1>;

    // Select implementation based on SSR and hardware support for symmetric pre-add
    static constexpr unsigned int useHBImpl = (__HAS_SYM_PREADD__ == 1); // Otherwise, use HB Asym
    static constexpr unsigned int useAsymImpl = (TP_PARA_DECI_POLY != 1);
    using lastSSRKernel = std::conditional_t<useAsymImpl,
                                             lastSSRKernelSrAsym,
                                             std::conditional_t<useHBImpl, lastSSRKernelHB, lastSSRKernelHBAsym> >;
   ...
   public:
     ...
     *                   coeff[]= {1,2,5,2,1}.
     **/
    static constexpr unsigned int rtpPortNumber =
        (TP_SSR * TP_SSR * TP_CASC_LEN) + (TP_PARA_DECI_POLY > 1 ? TP_SSR : 0);
    using rtp_port_array = port_conditional_array<input, (TP_USE_COEFF_RELOAD == 1), rtpPortNumber>;
    ...
    **/
    static constexpr unsigned int getTotalRtpPorts() {
        ...
        //
        return (TP_USE_COEFF_RELOAD == 0) ? 0 : rtpPortNumber;
    };
    ...
    **/
    static std::vector<TT_COEFF> extractTaps(const std::vector<TT_COEFF>& taps, unsigned int kernelNo) {
        // Debug print to see the taps array
        std::vector<TT_COEFF> ssrTaps;
        std::vector<TT_COEFF> ct_vector;
        if (useAsymImpl) {
            ct_vector.push_back(taps.at((TP_FIR_LEN + 1) / 4));
            ssrTaps = lastSSRKernel::convert_sym_taps_to_asym(((TP_FIR_LEN + 1) / 2), taps);
        } else if (useHBImpl) {
            ssrTaps = taps;

        } else {
            // Convert symmetric coefficients to full-length asymmetric form to handle both odd and even symmetry cases
            // std::vector<TT_COEFF> asymTaps =
            // lastSSRKernel::convert_sym_taps_to_asym(((TP_FIR_LEN + 1) / 2), taps);
            // asymTaps.push_back(taps.at((TP_FIR_LEN + 1) / 4));
            ssrTaps = lastSSRKernel::convert_sym_taps_to_asym(((TP_FIR_LEN + 1) / 2), taps);
            ssrTaps.push_back(taps.at((TP_FIR_LEN + 1) / 4));
        }
        // Debug print ssrTaps array
        unsigned int useCtVector = kernelNo >= (TP_SSR * TP_SSR * TP_CASC_LEN) ? 1 : 0;

        unsigned int ssrKernelIndex = (kernelNo / TP_CASC_LEN);
        unsigned int ssrInnerPhase = (ssrKernelIndex % TP_SSR);
        unsigned int ssrOutputPath = (ssrKernelIndex / TP_SSR);
        unsigned int ssrCoeffPhase = lastSSRKernel::getSSRCoeffPhase(ssrOutputPath, ssrInnerPhase, TP_SSR);
        std::vector<TT_COEFF> ssrPhaseTaps = lastSSRKernel::segment_taps_array_for_phase(ssrTaps, ssrCoeffPhase);
        // split over cascaded kernels
        std::vector<TT_COEFF> tapsForCascade;
        int kernelPosition = kernelNo % TP_CASC_LEN;
        int cascadeLength = TP_CASC_LEN;
        std::vector<TT_COEFF> kernelTaps;
        if (useCtVector == 1) {
            kernelPosition = 0;
            cascadeLength = 1;
            kernelTaps = lastSSRKernelCT::segment_taps_array_for_cascade(ct_vector, kernelPosition, cascadeLength);
        } else {
            kernelTaps = lastSSRKernel::segment_taps_array_for_cascade(ssrPhaseTaps, kernelPosition, cascadeLength);
        }
        // Make sure kernelTaps is always constructed through thisSSRKernel, as this method forces correct vector length
        // Debug print tapsForCascade array
        // coeffCT RTP port is flattened into one dimensional array of coeff RTP ports and added at the end.
        // When used, a single coeff will be broadcast to all kernels created with ct_kernels class.
        return kernelTaps;
    };
    ...
    void update_rtp(TopGraph& top, const std::vector<TT_COEFF>& taps, rtp_port_array& portArray) {
        ...
        for (unsigned int i = 0; i < rtpPortNumber; i++) {
            std::vector<COEFF_TYPE> tapsForRtpPort = extractTaps(taps, i);
...
#else
                std::vector<TT_COEFF> asymTaps = lastSSRKernel::convert_sym_taps_to_asym(((TP_FIR_LEN + 1) / 2), taps);
                asymTaps.push_back(taps.at((TP_FIR_LEN + 1) / 4));
                lastSSRKernel::create_and_recurse(m_firKernels, asymTaps);
#endif
            }
        else {
            // SSR>=1
            // rearrange taps
            std::vector<TT_COEFF> srTaps = lastSSRKernel::convert_sym_taps_to_asym(((TP_FIR_LEN + 1) / 2), taps);
            lastSSRKernel::create_and_recurse(m_firKernels, srTaps);
            std::vector<TT_COEFF> ct_vector;
            ct_vector.push_back(taps.at((TP_FIR_LEN + 1) / 4));
