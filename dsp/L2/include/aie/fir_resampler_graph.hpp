...
class fir_resampler_graph : public graph {
   private:
    ...
    // using lastSSRKernel = ssr_kernels<ssr_params, fir_resampler_tl>; // not correct. When decomposed, actual kernel
    // may be e.g. fir_sr_asym. Use decomposer class.
   ...
   public:
     ...
     **/
    using rtp_port_array =
        port_conditional_array<input,
                               (TP_USE_COEFF_RELOAD == 1),
                               (TP_PARA_INTERP_POLY * TP_PARA_DECI_POLY * TP_SSR * TP_SSR * TP_CASC_LEN)>;
    ...
    **/
    ...
    using decomposerClass = decomposer::polyphase_decomposer<ssr_params>;
    using lastSSRKernel = typename decomposerClass::lastSSRKernelOfStartingPolyphase;
    ...
    **/
    static constexpr unsigned int getTotalRtpPorts() {
        ...
        //
        return (TP_USE_COEFF_RELOAD == 0) ? 0
                                          : (TP_CASC_LEN * TP_SSR * TP_SSR * TP_PARA_DECI_POLY * TP_PARA_INTERP_POLY);
    };
    ...
    **/
    static std::vector<TT_COEFF> extractTaps(const std::vector<TT_COEFF>& taps, unsigned int kernelNo) {
        unsigned int interpPolyIdx = decomposerClass::getInterpPolyIdx(kernelNo);
        unsigned int deciPolyIdx = decomposerClass::getDeciPolyIdx(kernelNo);
        std::vector<TT_COEFF> tapsForPolyphase =
            decomposerClass::segment_taps_array_for_polyphase(taps, interpPolyIdx, deciPolyIdx);
        ...
        std::vector<TT_COEFF> ssrPhaseTaps =
            lastSSRKernel::segment_taps_array_for_phase(tapsForPolyphase, ssrCoeffPhase);
    ...
    fir_resampler_graph(const std::vector<TT_COEFF>& taps) : net{}, net2{} {
        decomposerClass::create(m_firKernels, taps);
        decomposerClass::create_connections(m_firKernels, &in[0], in2, &out[0], out2, coeff, net, net2, casc_in,
                                            srcFileName, useRtpPerKernel);
    };
    ...
    fir_resampler_graph() : net{}, net2{} {
        decomposerClass::create(m_firKernels);
        decomposerClass::create_connections(m_firKernels, &in[0], in2, &out[0], out2, coeff, net, net2, casc_in,
                                            srcFileName, useRtpPerKernel);
    };
