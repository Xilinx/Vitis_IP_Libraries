...
class fir_sr_sym_graph : public graph {
   private:
    ...
    void conditional_rtp_connections(port<input>(&coeff),
                                     kernel firKernels[TP_CASC_LEN],
                                     unsigned int rtpPortPos = TP_API == 0 ? TP_DUAL_IP + 1 : 1) {
    ...
    void create_connections() {
        ...
        for (unsigned int ssrOutputPath = 0; ssrOutputPath < TP_SSR; ssrOutputPath++) {
            for (unsigned int ssrInnerPhase = 0; ssrInnerPhase < TP_SSR; ssrInnerPhase++) {
                ...
                unsigned int ssrDataPhase = lastSSRKernelSym::getSSRDataPhase(ssrOutputPath, ssrInnerPhase, TP_SSR);
                unsigned int ssrCoeffPhase = lastSSRKernelSym::getSSRCoeffPhase(ssrOutputPath, ssrInnerPhase, TP_SSR);
                unsigned int kernelStartingIndex =
                    lastSSRKernelSym::getKernelStartingIndex(ssrDataPhase, ssrOutputPath, TP_SSR, TP_CASC_LEN);
                ...
                unsigned int nextChainKernelStartingIndex = lastSSRKernelSym::getKernelStartingIndex(
                    lastSSRKernelSym::getSSRDataPhase(ssrOutputPath, ssrInnerPhase + 1, TP_SSR), ssrOutputPath, TP_SSR,
                    TP_CASC_LEN);
                ...
                if
                    constexpr(TP_USE_COEFF_RELOAD == USE_COEFF_RELOAD_TRUE) {
                        // This method is only used in non-SSR context. Therefore, ssrInnerPhase is always 0.
                        // In addition, 1st m_firKernels will have only one input port when TP_API == 1, due to the
                        // widget usage.
                        for (int i = 0; i < TP_CASC_LEN; i++) {
                            // RTP position is after the buffer and cascade input ports.
                            // Cascade is present for first kernel in cascade chain, if part of a bigger SSR structure.
                            unsigned int rtpPosition = (i == 0 && ssrInnerPhase == 0)
                                                           ? CASC_IN_PORT_POS
                                                           : CASC_IN_PORT_POS + 1; // Start of cascade
    ...
    // Define type aliases for the last SSR kernel implementations:
    // - lastSSRKernelSym: for symmetric FIR implementation
    // - lastSSRKernelAsym: for asymmetric FIR implementation
    using lastSSRKernelSym = ssr_kernels<ssr_params, fir_sr_sym_tl>;
    ...
    static constexpr unsigned int useSymImpl = (TP_SSR == 1 && __HAS_SYM_PREADD__ == 1);
    using lastSSRKernel = std::conditional_t<(useSymImpl), lastSSRKernelSym, lastSSRKernelAsym>;

    // Define a static constexpr unsigned int for use elsewhere
    ...
    **/
    static constexpr unsigned int getTotalRtpPorts() {
        ...
        //
        return (TP_USE_COEFF_RELOAD == 0) ? 0 : (TP_SSR * TP_SSR * TP_CASC_LEN);
    };
    ...
    **/
    static std::vector<TT_COEFF> extractTaps(const std::vector<TT_COEFF>& taps, unsigned int kernelNo) {
        ...
        if (!useSymImpl) {
            ...
            ssrTaps = lastSSRKernel::convert_sym_taps_to_asym(TP_FIR_LEN, taps);
        } else {
            ssrTaps = taps;
        }

        unsigned int ssrKernelNo = (kernelNo / TP_CASC_LEN);
        unsigned int ssrInnerPhase = (ssrKernelNo % TP_SSR);
        unsigned int ssrOutputPath = (ssrKernelNo / TP_SSR);
        ...
        std::vector<TT_COEFF> kernelTaps =
            lastSSRKernel::segment_taps_array_for_cascade(ssrPhaseTaps, kernelPosition, TP_CASC_LEN, useSymImpl);
    ...
    fir_sr_sym_graph(const std::vector<TT_COEFF>& taps) {
        if
            constexpr(useSymImpl) {
                lastSSRKernel::create_and_recurse(m_firKernels, taps);
                // Use class specific connections
                create_connections();
            }
        else {
            std::vector<TT_COEFF> srAsymTaps = lastSSRKernel::convert_sym_taps_to_asym(TP_FIR_LEN, taps);
            lastSSRKernel::create_and_recurse(m_firKernels, srAsymTaps);
    ...
    fir_sr_sym_graph() {
        ...
        if
            constexpr(useSymImpl) { create_connections(); }
        else {
            lastSSRKernel::create_connections(m_firKernels, &in[0], in2, &out[0], out2, coeff, net, net2, casc_in,
                                              "fir_sr_asym.cpp", useRtpPerKernel);
        }
