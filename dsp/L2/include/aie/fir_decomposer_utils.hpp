...
class polyphase_decomposer {
   private:
    ...
    static constexpr unsigned int TP_USE_COEFF_RELOAD = p::BTP_USE_COEFF_RELOAD;
    static constexpr unsigned int TP_DUAL_IP = p::BTP_DUAL_IP;
    ...
    using decomposed_rtp_array =
        port_conditional_array<input, (p::BTP_USE_COEFF_RELOAD == 1), TP_SSR * TP_SSR * TP_CASC_LEN>;
    ...
    static void create_connections_and_recurse(
        ...
        interp_poly_array<deci_poly_array<decomposed_rtp_array> > decomposed_coeff,
        ...
        unsigned int rtpPerKernel = useRtpPerKernel) {
        unsigned int kernelIndex = getKernelIdx(interpPolyIdx, deciPolyIdx);
        ...
        if (deciPolyIdx == 0) {
            ...
            ssrKickOffInit<interpPolyIdx, deciPolyIdx>::create_connections(
                ...
                decomposed_coeff[interpPolyIdx][deciPolyIdx], net[interpPolyIdx][deciPolyIdx],
                net2[interpPolyIdx][deciPolyIdx], casc_in, resSrcFile, rtpPerKernel);
            ...
            if
                constexpr(TP_PARA_DECI_POLY > 2) {
                    ...
                    ssrKickOffMid<interpPolyIdx, deciPolyIdx>::create_connections(
                        ...
                        out2_no_port, decomposed_coeff[interpPolyIdx][deciPolyIdx], net[interpPolyIdx][deciPolyIdx],
                        net2[interpPolyIdx][deciPolyIdx], casc_in_deci_intermediate[interpPolyIdx][deciPolyIdx - 1],
                        resSrcFile, rtpPerKernel);
                }
            ...
            if
                constexpr(TP_PARA_DECI_POLY > 1) {
                    ssrKickOffLast<interpPolyIdx, deciPolyIdx>::create_connections(
                        &firKernels[kernelIndex], &in_ssr[interpPolyIdx][deciPolyIdx][0],
                        in2_ssr[interpPolyIdx][deciPolyIdx], &out_deci_intermediate[interpPolyIdx][deciPolyIdx][0],
                        out2_ssr[interpPolyIdx][deciPolyIdx], decomposed_coeff[interpPolyIdx][deciPolyIdx],
                        net[interpPolyIdx][deciPolyIdx], net2[interpPolyIdx][deciPolyIdx],
                        casc_in_deci_intermediate[interpPolyIdx][deciPolyIdx - 1], resSrcFile, rtpPerKernel);
                }
        }
        if
            constexpr(interpPolyIdx == 0 && deciPolyIdx == 0) {
            ...
            create_connections_and_recurse<nextInterpPolyIdx, nextDeciPolyIdx>(
                ...
                casc_in_deci_intermediate, net, net2, casc_in, rtpPerKernel);
        }
    }

   public:
    using lastSSRKernelOfStartingPolyphase = ssrKickOffLast<0, 0>;

    // Get interpolate polyphase index from kernel index
    static unsigned int getInterpPolyIdx(unsigned int kernelIdx) {
        return kernelIdx / (n_ssr_kernels * TP_PARA_DECI_POLY);
    }

    // Get decimate polyphase index from kernel index
    static unsigned int getDeciPolyIdx(unsigned int kernelIdx) {
        return (kernelIdx / n_ssr_kernels) % TP_PARA_DECI_POLY;
    }
    ...
    template <typename in2_type = port_conditional_array<input, (TP_DUAL_IP == 1), TP_SSR>,
              ...
              typename coeff_type =
                  port_conditional_array<input, (TP_USE_COEFF_RELOAD == 1), TP_SSR> // old scheme has an array of SSR,
                                                                                    // new scheme SSR^2*CASC_LEN,
              >
    static void create_connections(kernel* firKernels,
            ...
            for (unsigned int deciPolyIdx = 0; deciPolyIdx < TP_PARA_DECI_POLY; deciPolyIdx++) {
                ...
                if
                    constexpr((params::BTP_USE_COEFF_RELOAD == 1)) {
                        // connect<>(coeff[TP_PARA_DECI_POLY * TP_SSR * TP_SSR * TP_CASC_LEN * interpPolyIdx + TP_SSR *
                        // TP_SSR * TP_CASC_LEN * deciPolyIdx],
        ...
        for (unsigned int interpPolyIdx = 0; interpPolyIdx < TP_PARA_INTERP_POLY; interpPolyIdx++) {
            for (unsigned int deciPolyIdx = 0; deciPolyIdx < TP_PARA_DECI_POLY; deciPolyIdx++) {
                for (unsigned int outPath = 0; outPath < TP_SSR; outPath++) {
                    for (unsigned int inPhase = 0; inPhase < TP_SSR; inPhase++) {
                        for (unsigned int cascIdx = 0; cascIdx < TP_CASC_LEN; cascIdx++) {
                            int ssrCoeffIdx = TP_SSR * TP_CASC_LEN * outPath + TP_CASC_LEN * inPhase + cascIdx;
                            int flatCoeffIdx = TP_PARA_DECI_POLY * TP_SSR * TP_SSR * TP_CASC_LEN * interpPolyIdx +
                                               TP_SSR * TP_SSR * TP_CASC_LEN * deciPolyIdx + ssrCoeffIdx;
                            if
                                constexpr((params::BTP_USE_COEFF_RELOAD == 1)) {
                                    connect<>(coeff[flatCoeffIdx],
                                              decomposed_coeff[interpPolyIdx][deciPolyIdx][ssrCoeffIdx]);
                                }
                        }
                    }
                }
