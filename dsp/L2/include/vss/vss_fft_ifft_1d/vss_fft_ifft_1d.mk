...
AIETARGET  ?= hw
OUTPUT_DIR ?= ./
AGREEMENT_PATH ?= ${DSPLIB_IP_ROOT_DIR}/L2/include/vss/vss_fft_ifft_1d/agreement.txt
...
VSSFILE    = ${VSS}/${VSS}.vss
RTL_FFT_XO = $(OUTPUT_DIR)/fft_xo/parallel_fft.xo
...
# if values are not available in user params file
ifeq ($(VSS_MODE), -1)
VSS_MODE := 1
endif
...
${VSSCFG}: ${DSPLIB_IP_ROOT_DIR}/L2/include/vss/vss_fft_ifft_1d/vss_fft_ifft_1d_con_gen.py
	$(VITIS_PYTHON3) ${DSPLIB_IP_ROOT_DIR}/L2/include/vss/vss_fft_ifft_1d/vss_fft_ifft_1d_con_gen.py  --ssr ${SSR} --cfg_file_name ${VSSCFG} --vss_unit ${VSS} --version 1 --freqhz ${hls_freq} --add_front_transpose ${ADD_FRONT_TRANSPOSE} --add_back_transpose ${ADD_BACK_TRANSPOSE} --data_type ${DATA_TYPE} --aie_obj_name ${aie_name}

${VSSFILE}: $(VSS_DEPS)
	v++ --link --mode vss --part $(shell $(VITIS_PYTHON3) ${DSPLIB_IP_ROOT_DIR}/L2/include/vss/scripts/extract_param_cfg.py $(OUTPUT_DIR) "PART" "top" $(PARAMS_CFG)) --save-temps  --out_dir ${OUTPUT_DIR}/${VSS} --config $^
...
${VSSCFG}: ${DSPLIB_IP_ROOT_DIR}/L2/include/vss/vss_fft_ifft_1d/vss_fft_ifft_1d_hdl_con_gen.py
	$(VITIS_PYTHON3) ${DSPLIB_IP_ROOT_DIR}/L2/include/vss/vss_fft_ifft_1d/vss_fft_ifft_1d_hdl_con_gen.py  --ssr ${SSR} --cfg_file_name ${VSSCFG} --vss_unit ${VSS} --version 1 --freqhz ${hls_freq} --add_back_transpose ${ADD_BACK_TRANSPOSE}

${VSSFILE}: $(VSS_DEPS)
	v++ --link --mode vss --part $(shell $(VITIS_PYTHON3) ${DSPLIB_IP_ROOT_DIR}/L2/include/vss/scripts/extract_param_cfg.py $(OUTPUT_DIR) "PART" "top" $(PARAMS_CFG)) --save-temps  --out_dir ${OUTPUT_DIR}/${VSS} --config $^
endif

${RTL_FFT_XO}: license_check
	${XILINX_VIVADO}/bin/vivado -mode batch -source ${DSPLIB_IP_ROOT_DIR}/L2/include/vss/scripts/xo_scripts/gen_xo.tcl -tclargs ./fft_xo/parallel_fft.xo parallel_fft hw_emu ${DSPLIB_IP_ROOT_DIR} ${SSR} ${XILINX_VITIS}
