...
#include "fir_resampler_native_generated_graph/fir_resampler_generated_graph.h"
...
class test_graph : public graph {
   public:
    ...
    using uut = fir_resampler_native_generated_graph;
#else
    // Use Graph class directly for reference model
    using uut = dsplib::fir::resampler::UUT_GRAPH<DATA_TYPE,
                                                  COEFF_TYPE,
                                                  FIR_LEN,
                                                  INTERPOLATE_FACTOR,
                                                  DECIMATE_FACTOR,
    ...
    test_graph() : firGraph(m_taps_v) {
        ...
        for (int i = 1; i < CASC_LEN; i++) {
            connect<stream, stream>* net = firGraph.filter.getInNet(i);
            fifo_depth(*net) = 384 + 16 * i;
#if (DUAL_IP == 1)
            connect<stream, stream>* net2 = firGraph.filter.getIn2Net(i);
        ...
        if (firGraph.filter.getKernels() != NULL) {
            ...
            printf("%d", arch);
            printf("]\n");
            printf("polyphaseLaneAlias = %d\n", firGraph.filter.getPolyphaseLaneAlias());
        }
#endif
    };
};
