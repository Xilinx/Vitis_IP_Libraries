...
class fir_tdm_ref_graph : public graph {
    ...
    **/
    ...
    /**
     * @brief Spoofed function that returns the input vector.
     * Reference models handle taps internally to the kernel and no action is needed here.
    ...
    *TP_CASC_LEN`).
    **/
    static std::vector<TT_COEFF> extractTaps(const std::vector<TT_COEFF>& taps, unsigned int kernelNo) { return taps; };
    ...
    //     for (unsigned int i = 0; i < rtpPortNumber; i++) {
    //         std::vector<COEFF_TYPE> tapsForRtpPort = extractTaps(taps, i);
    //         top.update(m_firKernel.in[1], tapsForRtpPort.data(), tapsForRtpPort.size());
    //     }
    // }
