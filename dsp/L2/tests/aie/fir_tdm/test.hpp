...
// FIR sub-graph
        ...
        // Connect PLIOs with UUT input ports
        ...
        if ((bufferSize > MAX_PING_PONG_SIZE) || (SINGLE_BUF == 1 && PORT_API == 0)) {
            for (int ssr = 0; ssr < P_SSR; ssr++) {
                for (int casc_len = 0; casc_len < CASC_LEN; casc_len++) {
                    ...
                    if (DUAL_IP == 1) {
                        single_buffer(firGraph.filter.getKernels(CASC_LEN * ssr + casc_len)->in[1]);
                    }
                ...
                if (NUM_OUTPUTS == 2) {
                    single_buffer(firGraph.filter.getKernels()[CASC_LEN * ssr + CASC_LEN - 1].out[1]);
                }
