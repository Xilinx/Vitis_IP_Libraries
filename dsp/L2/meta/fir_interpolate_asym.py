...
def generate_graph(graphname, args):
    ...
    coeff_ip_connect_str = (
        f"""for (int i = 0; i < rtpPortNumber; i++) {{
            adf::connect<>(coeff[i], filter.coeff[i]);
        }}"""
        if TP_USE_COEFF_RELOAD == 1
        else "// No coeff port"
    )
...
public:
    ...
    using ssr_port_array = std::array<adf::port<dir>, TP_SSR*TP_PARA_INTERP_POLY>;

    std::vector<{TT_COEFF}> taps = {taps};
    using firClass = xf::dsp::aie::fir::interpolate_asym::fir_interpolate_asym_graph<
        {TT_DATA}, //TT_DATA
        {TT_COEFF}, //TT_COEFF
