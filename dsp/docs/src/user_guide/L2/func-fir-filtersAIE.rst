...
FIR taps will be split and distributed to all kernels, by ``TP_CASC_LEN``. Each part is sent to a specific kernel via its corresponding RTP port.
...
FIR graph class provides a helper method: ``getTotalRtpPorts()`` to get number of RTP ports. For more details, refer to: :ref:`API_REFERENCE`.
...
FIR graph class provides a helper method: ``extractTaps(const std::vector<TT_COEFF>& taps, unsigned int kernelNo)`` to get coefficients for a given kernel. For more details, refer to: :ref:`API_REFERENCE`.
