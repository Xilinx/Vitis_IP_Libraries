...
#pragma unroll(kParallelPhases)
                        for (int phase = 0; phase < kParallelPhases; ++phase) {
                            T_buff_256b<TT_COEFF>* coeff =
                                ...
                                phase * tapsArraySize / m_kCoeffRegVsize;
                            coe[phase] = *coeff;
                        }
...
#pragma unroll(kParallelPhases)
                        for (int phase = 0; phase < kParallelPhases; ++phase) {
                            if ((m_kColumns * op) % m_kCoeffRegVsize == 0) {
                                T_buff_256b<TT_COEFF>* coeff =
                                    ((T_buff_256b<TT_COEFF>*)m_internalTaps /* m_internalTaps2 */ +
                                     phase * tapsArraySize / m_kCoeffRegVsize + (m_kColumns * op) / m_kCoeffRegVsize);
                                coe[phase] = *coeff;
                            }
...
#pragma unroll(m_kPhases)
                ...
                for (int phase = 0; phase < m_kPhases; ++phase) {
                    coeff = ((T_buff_256b<TT_COEFF>*)m_internalTaps +
                             (LCMPhase * m_kLanes * m_kColumns * m_kNumOps) / m_kCoeffRegVsize);
                    coe0 = *coeff;
...
#pragma unroll(GUARD_ZERO((m_kNumOps - 1)))
                    for (int op = 1; op < m_kNumOps; ++op) {
                        coeff = ((T_buff_256b<TT_COEFF>*)m_internalTaps +
                                 (LCMPhase * m_kLanes * m_kColumns * m_kNumOps) / m_kCoeffRegVsize +
                                 (op * m_kLanes * m_kColumns) / m_kCoeffRegVsize);
                        coe0 = *coeff;
                        xstart += m_kColumns;
                        // perform the MAC, i.e. cacculate some terms of the FIR polynomial
                        acc = macIntAsym(acc, sbuff, coe0, TP_INTERPOLATE_FACTOR, m_kLanes, m_dataOffsets[LCMPhase],
