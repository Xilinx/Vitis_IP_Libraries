...
INLINE_DECL void kernelFilterClass<TT_DATA,
    ...
    // sendRtpTrigger(m_coeffnEq, outInterface);
    // if (m_coeffnEq) { // Coefficients have changed
    //     bufferReload<TT_DATA, TT_COEFF, (TP_FIR_LEN + 1) / 4 + 1, TP_DUAL_IP>(inInterface, m_oldInTaps,
    //     outInterface);
...
INLINE_DECL void kernelFilterClass<TT_DATA,
    ...
    T_buff_256b<TT_COEFF>* __restrict coeffCt =
        (T_buff_256b<TT_COEFF>*)m_internalTaps + internalCTIndex / m_kCoeffRegVsize;
...
#pragma unroll(GUARD_ZERO((m_kNumOps - 1)))
                for (int op = 1; op < m_kNumOps; ++op) {
                    ...
                    if ((m_kNumOps - op) % (m_kCoeffRegVsize / m_kColumns) == 0) {
                        ...
                        coeff = ((T_buff_256b<TT_COEFF>*)m_internalTaps +
                                 ((m_kNumOps - op) - (m_kCoeffRegVsize / m_kColumns)) * m_kColumns / m_kCoeffRegVsize);
                        coe0 = *coeff;
                    }
