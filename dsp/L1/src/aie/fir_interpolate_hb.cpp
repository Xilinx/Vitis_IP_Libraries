...
INLINE_DECL void kernelFilterClass<TT_DATA,
                                   ...
                                   TP_SAT>::filterSelectArch(T_inputIF<TP_CASC_IN, TT_DATA, TP_DUAL_IP> inInterface,
                                                             T_outputIF<TP_CASC_OUT, TT_DATA> outInterface) {
    // TODO: This is temporary and incorrect. m_internalTaps may only contain a fraction of the array
    // and may not contain (TP_FIR_LEN + 1) / 4 elements.

    if (TP_UPSHIFT_CT == 0) {
        m_phaseTwoTap[0] = m_internalTaps[InternalCenterTapIndex];
    } else {
        m_ctShift = getUpshiftCt(m_internalTaps[InternalCenterTapIndex]);
    }
