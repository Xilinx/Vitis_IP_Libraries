...
class fir_sr_asym_tl {
   public:
    ...
    static constexpr unsigned int getKernelFirRangeLen(int pos) {
        return pos + 1 == fp::BTP_CASC_LEN ? fnFirRangeRemAsym<typename fp::BTT_DATA, typename fp::BTT_COEFF>(
                                                 fp::BTP_FIR_LEN, fp::BTP_CASC_LEN, pos, fp::BTP_API)
                                           : fnFirRangeAsym<typename fp::BTT_DATA, typename fp::BTT_COEFF>(
                                                 fp::BTP_FIR_LEN, fp::BTP_CASC_LEN, pos, fp::BTP_API);
    };
    ...
    static constexpr unsigned int getFirRangeTapLen(int pos) {
        return CEIL(getKernelFirRangeLen(pos), fp::BTP_SSR) / fp::BTP_SSR;
    };
...
// TODO - express this more elegantly
    ...
    static constexpr unsigned int internalTapsSize = CEIL(TP_FIR_RANGE_LEN, m_kCoeffLoadSize);
    ...
    static std::vector<TT_COEFF> firReload(const std::vector<TT_COEFF>& taps) {
        ...
        for (int i = 0; i < TP_FIR_RANGE_LEN; i++) {
            unsigned int tapsAddress =
                TP_FIR_LEN - 1 -
                fnFirRangeOffsetAsym<TP_FIR_LEN, TP_CASC_LEN, TP_KERNEL_POSITION, TT_DATA, TT_COEFF, TP_API>() - i;
            internalTaps[i] = (tapsAddress < taps.size()) ? taps[tapsAddress] : nullElem<TT_COEFF>();
        }
        return internalTaps;
    };
...
class fir_sr_asym : public kernelFilterClass<TT_DATA,
   ...
   public:
    ...
    using thisKernelFilterClass = kernelFilterClass<TT_DATA,
                                                    ...
                                                    TP_COEFF_PHASES_LEN,
                                                    TP_SAT>;
    fir_sr_asym(TT_COEFF (&taps)[TP_FIR_LEN]) : thisKernelFilterClass(taps) {}
...
class fir_sr_asym<TT_DATA,
   ...
   public:
    ...
    fir_sr_asym(TT_COEFF (&taps)[internalTapsSize]) : internalTaps(taps), thisKernelFilterClass(taps){};
