...
class fir_interpolate_asym_tl {
   public:
    ...
    static constexpr unsigned int getFirRangeTapLen() { return fp::BTP_FIR_RANGE_LEN + 1; };
    ...
    // Get FIR source file
    static const char* getFirSource() { return "fir_interpolate_asym.cpp"; };
...
class kernelFilterClass {
   private:
    ...
    static constexpr unsigned int tapsSizePermuteSupported = m_kLCMPhases * m_kNumOps * m_kColumns * m_kLanes;
    static constexpr unsigned int tapsSizePermuteNotSupported = m_kPhases * tapsArraySize;
   ...
   public:
                ...
                for (int op = 0; op < m_kNumOps; ++op) {
                    for (int column = 0; column < m_kColumns; ++column) {
                        for (int lane = 0; lane < m_kLanes; ++lane) {
                            ...
                            // Flatten the 4D m_internalTaps array into a 1D internalTaps vector
                            int flatIndex = phase * (m_kNumOps * m_kColumns * m_kLanes) + op * (m_kColumns * m_kLanes) +
                                            column * m_kLanes + lane;
                            if (tapIndex < TP_FIR_RANGE_LEN && tapIndex >= 0) {
                                tapIndex = TP_FIR_LEN - 1 - tapIndex -
                                           fnFirRangeOffset<TP_FIR_LEN, TP_CASC_LEN, TP_KERNEL_POSITION,
                                                            TP_INTERPOLATE_FACTOR>(); // Reverse coefficients and apply
                                ...
                                // head of file.
                                internalTaps[flatIndex] = taps[tapIndex];

                            } else {
                                internalTaps[flatIndex] = nullElem<TT_COEFF>(); // 0 for the type.
                            }
                        }
                    }
                }
            ...
            for (int phase = 0; phase < m_kPhases; ++phase) {
                for (int i = 0; i < tapsArraySize; i++) { // ceiled to columns.
                    ...
                    int tapsAddress =
                        ...
                        tapIndex;
                    if (tapsAddress < 0 || tapIndex >= TP_FIR_RANGE_LEN) {
                        internalTaps[phase * tapsArraySize + i] = nullElem<TT_COEFF>();
                    } else {
                        internalTaps[phase * tapsArraySize + i] = taps[tapsAddress];
                    }
                }
            }
        }
    ...
    //                                                                TP_INTERPOLATE_FACTOR>(); // Reverse coefficients
    //                                                                and
    //                                                                                          // apply cascade range
    //                                                                                          offset.
    //                                                                                          // See note at head of
...
class fir_interpolate_asym : public kernelFilterClass<TT_DATA,
   ...
   public:
    ...
    fir_interpolate_asym(TT_COEFF (&taps)[internalTapsSize]) : internalTaps(taps), thisKernelFilterClass(taps){};

    // Register Kernel Class
    static void registerKernelClass() {
        REGISTER_PARAMETER(internalTaps);
        REGISTER_FUNCTION(fir_interpolate_asym::filter);
    }
...
class fir_interpolate_asym<TT_DATA,
   ...
   public:
    using thisKernelFilterClass = kernelFilterClass<TT_DATA,
                                                    ...
                                                    TP_SAT>;
    static constexpr unsigned int internalTapsSize = thisKernelFilterClass::get_internalTapsSize();
    fir_interpolate_asym() : thisKernelFilterClass(){};
