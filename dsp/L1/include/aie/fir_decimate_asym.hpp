...
class fir_decimate_asym_tl {
   public:
    ...
    template <int pos>
    static constexpr unsigned int getKernelFirRangeLen() {
        return getKernelFirRangeLen(pos);
    };

    // Reload method for FIR coefficients using parent class functionality
    static std::vector<typename fp::BTT_COEFF> firReload(const std::vector<typename fp::BTT_COEFF>& taps) {
        return parent_class::firReload(taps);
    }
    ...
    // Get kernel's FIR range tap vector length
    static constexpr unsigned int getFirRangeTapLen() { return fp::BTP_FIR_RANGE_LEN; };
...
class kernelFilterClass {
   private:
    ...
    unsigned int m_kDecimateOffsets = setDecimateOffsets();
    ...
    //     m_rawInTaps[CEIL(TP_COEFF_PHASES_LEN, m_kCoeffLoadSize)]; // Previous user input coefficients with zero
    //     padding
    ...
    // alignas(__ALIGN_BYTE_SIZE__)  TT_COEFF m_internalTaps[CEIL(TP_FIR_RANGE_LEN, m_kCoeffLoadSize)]; // Filter
    // taps/coefficients
    static constexpr int tapsArraySize = CEIL(CEIL(TP_FIR_RANGE_LEN, TP_DECIMATE_FACTOR) / TP_DECIMATE_FACTOR,
                                              m_kCoeffLoadSize); // ceil'ed to 256-bits
    ...
    static constexpr unsigned int tapsSizePermuteSupported = CEIL(TP_FIR_RANGE_LEN, m_kCoeffLoadSize);
    static constexpr unsigned int tapsSizePermuteNotSupported = TP_DECIMATE_FACTOR * tapsArraySize;
    static constexpr unsigned int internalTapsSize =
        m_kPermuteSupport == 1 ? tapsSizePermuteSupported : tapsSizePermuteNotSupported;

   public:
    TT_COEFF* __restrict m_internalTaps;
    TT_COEFF* __restrict m_internalTaps2;
    ...
    // Constructor
    kernelFilterClass(TT_COEFF (&taps)[internalTapsSize]) : m_internalTaps{taps}, m_internalTaps2{taps} {}

    // RTP Constructor
    kernelFilterClass() : m_internalTaps(nullptr), m_internalTaps2(nullptr) {}
    ...
    // kernelFilterClass(const TT_COEFF (&taps)[internalTapsSize]) : m_internalTaps{} {
    //     setDecimateOffsets();
    ...
    // setDecimateOffsets
    unsigned int setDecimateOffsets() {
        unsigned int decimateOffsets;
        switch (TP_DECIMATE_FACTOR) {
            case 2:
                decimateOffsets = 0xECA86420;
                break; // No point in hi because range is exceeded.
            case 3:
                decimateOffsets = m_kDFX == kLowDF ? 0x9630 : m_kColumns == 1 ? 0x3030 : 0xA9764310;
    ...
    //             which is
    //             //  inefficient.
    //             // TODO: Stick 2 int16 into an int32 to avoid it, for an example, look at fir_resampler kernel's
    //             firReload
    ...
    static std::vector<TT_COEFF> firReload(const std::vector<TT_COEFF>& taps) {
        std::vector<TT_COEFF> internalTaps(internalTapsSize, nullElem<TT_COEFF>());
        ...
        if
            constexpr(m_kPermuteSupport == 1) {
                ...
                for (int i = 0; i < TP_FIR_RANGE_LEN; i++) {
                    unsigned int tapsAddress =
                        TP_FIR_LEN - 1 -
                        fnFirRangeOffset<TP_FIR_LEN, TP_CASC_LEN, TP_KERNEL_POSITION, TP_DECIMATE_FACTOR>() - i;
                    if (tapsAddress < taps.size()) {
                        internalTaps[i] = taps[tapsAddress];
                    } else {
                        internalTaps[i] = nullElem<TT_COEFF>();
                    }
                }
            }
        else {
            for (int phase = 0; phase < TP_DECIMATE_FACTOR; ++phase) {
                for (int i = 0; i < tapsArraySize; i++) { // ceiled to columns.
                    ...
                    int resultIndex = phase * tapsArraySize + i;

                    if (tapsAddress < 0 || tapIndex >= TP_FIR_RANGE_LEN || tapsAddress >= taps.size()) {
                        internalTaps[resultIndex] = nullElem<TT_COEFF>();
                    } else {
                        internalTaps[resultIndex] = taps[tapsAddress];
                    }
                }
            }
        }
...
class fir_decimate_asym : public kernelFilterClass<TT_DATA,
   ...
   public:
    ...
    fir_decimate_asym(TT_COEFF (&taps)[internalTapsSize]) : internalTaps(taps), thisKernelFilterClass(taps){};

    // Register Kernel Class
    static void registerKernelClass() {
        REGISTER_PARAMETER(internalTaps);
        REGISTER_FUNCTION(fir_decimate_asym::filter);
    }
...
class fir_decimate_asym<TT_DATA,
   ...
   public:
    ...
    static constexpr unsigned int internalTapsSize = thisKernelFilterClass::get_internalTapsSize();
    fir_decimate_asym() : thisKernelFilterClass(){};
