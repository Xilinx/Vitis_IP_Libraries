...
class fir_resampler_tl {
   public:
    ...
    static constexpr unsigned int getKernelFirRangeLen(int pos) {
        return pos + 1 == fp::BTP_CASC_LEN ? fnFirRangeRem(CEIL(fp::BTP_FIR_LEN, fp::BTP_INTERPOLATE_FACTOR),
    ...
    static constexpr unsigned int getFirRangeTapLen() {
        return CEIL(fp::BTP_FIR_RANGE_LEN, fp::BTP_SSR) / fp::BTP_SSR;
    };
...
class kernelFilterClass {
   private:
    ...
    static constexpr unsigned int tapsSizePermuteSupported = m_kPolyphaseCoeffAlias * m_kNumOps * m_kColumns * m_kLanes;
    static constexpr unsigned int tapsSizePermuteNotSupported = kParallelPhases * tapsArraySize;
   ...
   public:
    ...
    //                                 int polyphaseIndex = (((coeffPhase * m_kLanes + poly) * D) %
    //                                 TP_INTERPOLATE_FACTOR);
    ...
    //                                     // 256b boundary.
    //                                     m_internalTaps[coeffPhase][op][col][poly] = nullElem<TT_COEFF>(); // 0 for
    //                                     the type.
    //                                 }
    ...
    //                     //  Scalar loads and stores are 32-bit granularity, using int16 directly causes
    //                     read-modify-write
    ...
    //                                     (TP_INTERPOLATE_FACTOR - 1 - (TP_DECIMATE_FACTOR * iphase) %
    //                                     TP_INTERPOLATE_FACTOR);
    ...
    static std::vector<TT_COEFF> firReload(const std::vector<TT_COEFF>& taps) {
        std::vector<TT_COEFF> internalTaps;
        if
            constexpr(m_kPermuteSupport == 1) {
                constexpr unsigned int I = TP_INTERPOLATE_FACTOR;
                constexpr unsigned int D = TP_DECIMATE_FACTOR;
                        ...
                        for (int col = 0; col < m_kColumns; col++) {
                            for (int poly = 0; poly < m_kLanes; poly++) {
                                ...
                                if (poly < TP_INTERPOLATE_FACTOR &&
                                    tapIndexRev >= CEIL(TP_FIR_LEN, TP_INTERPOLATE_FACTOR) - m_kFirCoeffOffset -
                                                       CEIL(TP_FIR_RANGE_LEN, TP_INTERPOLATE_FACTOR) &&
                                    tapIndexRev < TP_FIR_LEN && tapIndexRev >= 0 && tapIndexRev < taps.size()) {
                                    val = taps[tapIndexRev];
                                } else {
                                    val = nullElem<TT_COEFF>();
                                }
                                internalTaps.push_back(val);
                            }
                        }
                    }
                }
...
class fir_resampler : public kernelFilterClass<TT_DATA,
   ...
   public:
    ...
    fir_resampler(TT_COEFF (&taps)[internalTapsSize]) : internalTaps(taps), thisKernelFilterClass(taps){};

    // Register Kernel Class
    static void registerKernelClass() {
        REGISTER_PARAMETER(internalTaps);
        REGISTER_FUNCTION(fir_resampler::filter);
    }
...
class fir_resampler<TT_DATA,
   ...
   public:
    ...
    static constexpr unsigned int internalTapsSize = thisKernelFilterClass::get_internalTapsSize();
    fir_resampler() : thisKernelFilterClass(){};
