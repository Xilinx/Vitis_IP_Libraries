...
class kernelFilterClass {
   private:
    ...
    // alignas(__ALIGN_BYTE_SIZE__)  TT_COEFF m_internalTaps[CEIL((TP_FIR_LEN + 1) / 4 + 1, m_kCoeffLoadSize)]; //
    ...
    static constexpr unsigned int internalTapsSize =
        CEIL((TP_FIR_RANGE_LEN + 1) / 4 + m_kCoeffLoadSize, m_kCoeffLoadSize);
    static constexpr unsigned int InternalCenterTapIndex = CEIL((TP_FIR_RANGE_LEN + 1) / 4, m_kCoeffLoadSize);
   ...
   public:
    ...
    //     } else {
    //         // Upshift Center tap.
    //         // Only supported for 16-bit integers. Accepted range: 0 - 16. Extracted from Real part when Complex
    //         numbers
    //         // used, hence the cast.
    ...
    static std::vector<TT_COEFF> firReload(const std::vector<TT_COEFF>& taps) {
        ...
        // Coefficients are pre-arranged, so that
        for (int i = 0; i < CEIL(((TP_FIR_RANGE_LEN + 1) / 4), m_kCoeffLoadSize); i++) {
            if (i < ((TP_FIR_RANGE_LEN + 1) / 4) &&
                (fnFirRangeOffsetSym<TP_FIR_LEN, TP_CASC_LEN, TP_KERNEL_POSITION>() / 2 + i) < taps.size()) {
                internalTaps[i] = taps[fnFirRangeOffsetSym<TP_FIR_LEN, TP_CASC_LEN, TP_KERNEL_POSITION>() / 2 + i];
            } else {
                internalTaps[i] = nullElem<TT_COEFF>();
            }
        }
        internalTaps[InternalCenterTapIndex] = taps[(TP_FIR_LEN + 1) / 4];

        // Optionally handle m_phaseTwoTap and m_ctShift if needed, but not returned in vector
        return internalTaps;
    }
...

{
   public:
    ...
    // Constructor
    fir_interpolate_hb(TT_COEFF (&taps)[internalTapsSize]) : internalTaps(taps), thisKernelFilterClass(taps) {}
...
class fir_interpolate_hb<TT_DATA,
   ...
   public:
    ...
    static constexpr unsigned int internalTapsSize = thisKernelFilterClass::get_internalTapsSize();
    // Constructor
    fir_interpolate_hb() : thisKernelFilterClass() {}
