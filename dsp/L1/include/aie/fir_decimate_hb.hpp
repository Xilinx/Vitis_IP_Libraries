...
class kernelFilterClass {
   private:
    ...
    // alignas(__ALIGN_BYTE_SIZE__)  TT_COEFF m_oldInTaps[CEIL((TP_FIR_LEN + 1) / 4 + 1,
   ...
   public:
    ...
    static constexpr unsigned int ctTapsSize = m_kCtPresent == 1 ? m_kCoeffRegVsize : 0;
    static constexpr unsigned int internalCTIndex =
        CEIL(m_kNumOps * m_kColumns, m_kCoeffRegVsize); // separate out center tap from the rest
    static constexpr unsigned int internalTapsSize = CEIL(m_kNumOps * m_kColumns, m_kCoeffRegVsize) + ctTapsSize;
   ...
   public:
    ...
    // Constructor
    kernelFilterClass(TT_COEFF (&taps)[internalTapsSize]) : m_internalTaps{taps} {}

    // RTP Constructor
    kernelFilterClass() : m_internalTaps(nullptr) {}
    ...
    //         // To avoid this hazard, the class has its own taps array which is zero-padded to the column width for
    //         the type
    //         // of coefficient.
    //         int tapIndex;
    //         int op;
    ...
    //         // Coefficients are pre-arranged such that during filter execution they may simply be read from a lookup
    //         table.
    //         for (op = 0; op < m_kNumOps; ++op) {
    //             for (column = 0; column < m_kColumns; ++column) {
    //                 tapIndex = (op * m_kColumns + column);
    //                 if (tapIndex < (TP_FIR_RANGE_LEN + 1) / 4) { // symmetry and halfband mean only 1/4 coeffs
    //                 matter. CT
    //                                                              // coeff ignored here, as it gets special handling
    //                                                              below.
    ...
    //                               : (TP_FIR_RANGE_LEN - 1) / 4; // Max tap index, only last kernel in cascade gets
    //                               CT.
    ...
    static std::vector<TT_COEFF> firReload(const std::vector<TT_COEFF>& taps) {
        ...
        for (op = 0; op < m_kNumOps; ++op) {
            for (column = 0; column < m_kColumns; ++column) {
                ...
                if (tapIndex < (TP_FIR_RANGE_LEN + 1) / 4) { // symmetry and halfband mean only 1/4 coeffs matter. CT
                    ...
                    internalTaps[tapIndex] =
                        taps[tapIndex + fnFirRangeOffsetSym<TP_FIR_LEN, TP_CASC_LEN, TP_KERNEL_POSITION>() / 2];
                } else {
                    internalTaps[tapIndex] = nullElem<TT_COEFF>(); // 0 for the type.
                }
            }
        }
        int tapIndexMin = TRUNC((TP_FIR_RANGE_LEN + 1) / 4, m_kColumns);
        ...
        for (column = m_kColumns - 1; column >= 0; column--) {
            if (tapIndex >= tapIndexMin && tapIndex <= tapIndexMax) {
                internalTaps[internalCTIndex + column] =
                    taps[tapIndex + fnFirRangeOffsetSym<TP_FIR_LEN, TP_CASC_LEN, TP_KERNEL_POSITION>() / 2];
            } else {
                internalTaps[internalCTIndex + column] = nullElem<TT_COEFF>();
            }
            tapIndex--;
        }
        return internalTaps;
...
class fir_decimate_hb : public kernelFilterClass<TT_DATA,
   ...
   public:
    ...
    // Constructor
    fir_decimate_hb(TT_COEFF (&taps)[internalTapsSize]) : internalTaps(taps), thisKernelFilterClass(taps) {}
...
class fir_decimate_hb<TT_DATA,
   ...
   public:
    ...
    static constexpr unsigned int internalTapsSize = thisKernelFilterClass::get_internalTapsSize();
    // Constructor
    fir_decimate_hb() : thisKernelFilterClass() {}
   ...
   public:
    ...
    static constexpr unsigned int getKernelFirRangeLen(int pos) {
        return pos + 1 == fp::BTP_CASC_LEN
                   ? (fp::BTP_POLY_SSR == 1 ? fnFirRangeRemSym(fp::BTP_FIR_LEN, fp::BTP_CASC_LEN, pos)
                                            : sr_asym::fnFirRangeRemAsym<typename fp::BTT_DATA, typename fp::BTT_COEFF>(
                                                  fp::BTP_FIR_LEN, fp::BTP_CASC_LEN, pos, fp::BTP_API))
                   : (fp::BTP_POLY_SSR == 1 ? fnFirRangeSym(fp::BTP_FIR_LEN, fp::BTP_CASC_LEN, pos)
                                            : sr_asym::fnFirRangeAsym<typename fp::BTT_DATA, typename fp::BTT_COEFF>(
                                                  fp::BTP_FIR_LEN, fp::BTP_CASC_LEN, pos, fp::BTP_API));
    };
    ...
    static constexpr unsigned int getFirRangeTapLen() {
        return CEIL((fp::BTP_FIR_RANGE_LEN + 1) / 4 + 1, fp::BTP_SSR) / fp::BTP_SSR;
    };
