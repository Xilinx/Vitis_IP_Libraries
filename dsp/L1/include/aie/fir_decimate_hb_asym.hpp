...
class fir_decimate_hb_asym_tl {
   public:
    ...
    static constexpr unsigned int getKernelFirRangeLen(int pos) {
        return pos + 1 == fp::BTP_CASC_LEN ? fnFirRangeRem(fp::BTP_FIR_LEN, fp::BTP_CASC_LEN, pos, 2)
                                           : fnFirRange(fp::BTP_FIR_LEN, fp::BTP_CASC_LEN, pos, 2);
    };
    ...
    static constexpr unsigned int getFirRangeTapLen() {
        ...
        return (fp::BTP_FIR_RANGE_LEN + 1) / 4 + 1;
#else
        return (fp::BTP_FIR_RANGE_LEN + 1) / 2 + 1;
#endif
    };
...
class kernelFilterClass {
   private:
    ...
    static constexpr unsigned int m_kZbuffSize = 256; // bits
    ...
    // alignas(__ALIGN_BYTE_SIZE__)  TT_COEFF m_internalTaps[CEIL(m_kNumAsymPh1Taps, m_kCoeffRegVsize)]; // Filter
    // taps/coefficients
    // alignas(__ALIGN_BYTE_SIZE__)  TT_COEFF m_phaseTwoTap[m_kCoeffRegVsize] = {
    //     nullElem<TT_COEFF>()}; // note, the array is initializeed, causing extra instructions during initialization.
    ...
    static constexpr unsigned int ctTapsSize = m_kHasCT == 1 ? m_kCoeffRegVsize : 0;
    static constexpr unsigned int internalCTIndex =
        CEIL(m_kNumAsymPh1Taps, m_kCoeffRegVsize); // separate out center tap from the rest
    static constexpr unsigned int internalTapsSize = CEIL(m_kNumAsymPh1Taps, m_kCoeffRegVsize) + ctTapsSize;
   ...
   public:
    ...
    static std::vector<TT_COEFF> firReload(const std::vector<TT_COEFF>& taps) {
        ...
        // multiple columns in the mul intrinsic, so the other columns must be zero'd
        if (m_kHasCT == 1) {
            internalTaps[internalCTIndex] = taps[m_kTotTaps - 1];
        }
        return internalTaps;
    }
...
class fir_decimate_hb_asym : public kernelFilterClass<TT_DATA,
   ...
   public:
    ...
    // Constructor
    fir_decimate_hb_asym(TT_COEFF (&taps)[internalTapsSize]) : internalTaps(taps), thisKernelFilterClass(taps) {}

    // Register Kernel Class
    static void registerKernelClass() {
        REGISTER_PARAMETER(internalTaps);
        REGISTER_FUNCTION(fir_decimate_hb_asym::filter);
    }
...
class fir_decimate_hb_asym<TT_DATA,
   ...
   public:
    ...
    static constexpr unsigned int internalTapsSize = thisKernelFilterClass::get_internalTapsSize();
    // Constructor
    fir_decimate_hb_asym() : thisKernelFilterClass() {}
