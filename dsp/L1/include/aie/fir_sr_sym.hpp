...
class kernelFilterClass {
   private:
    ...
    // TODO: This is correct for asymmetric, symmetric can have this simplified.
    ...
    static constexpr unsigned int m_kCoeffLoadSize = __MAX_READ_WRITE__ / 8 / sizeof(TT_COEFF);
    ...
    // alignas(__ALIGN_BYTE_SIZE__)  TT_COEFF m_internalTaps[CEIL((TP_FIR_RANGE_LEN + 1) / kSymmetryFactor,
    // m_kCoeffLoadSize)];
    ...
    static constexpr unsigned int internalTapsSize = CEIL((TP_FIR_RANGE_LEN + 1) / kSymmetryFactor, m_kCoeffLoadSize);
   ...
   public:
    ...
    static std::vector<TT_COEFF> firReload(const std::vector<TT_COEFF>& taps) {
        ...
        for (int i = 0; i < CEIL((TP_FIR_RANGE_LEN + 1) / kSymmetryFactor, m_kColumns); i++) {
            if (i < (TP_FIR_RANGE_LEN + 1) / kSymmetryFactor) {
                size_t tapIndex = i + fnFirRangeOffsetSym<TP_FIR_LEN, TP_CASC_LEN, TP_KERNEL_POSITION>();
                internalTaps[i] = (tapIndex < taps.size()) ? taps[tapIndex] : nullElem<TT_COEFF>();
            } else {
                internalTaps[i] = nullElem<TT_COEFF>(); // 0 for the type.
            }
        }
        return internalTaps;
    }
...
class fir_sr_sym : public kernelFilterClass<TT_DATA,
   ...
   public:
    ...
    // Constructor
    fir_sr_sym(TT_COEFF (&taps)[internalTapsSize]) : internalTaps(taps), thisKernelFilterClass(taps) {}

    // Register Kernel Class
    static void registerKernelClass() {
        REGISTER_PARAMETER(internalTaps);
        REGISTER_FUNCTION(fir_sr_sym::filter);
    }
    // FIR
...
class fir_sr_sym<TT_DATA,
   ...
   public:
    ...
    static constexpr unsigned int internalTapsSize = thisKernelFilterClass::get_internalTapsSize();
    // Constructor
    fir_sr_sym() : thisKernelFilterClass() {}
...
class fir_sr_sym_tl : public fir_sr_sym<typename fp::BTT_DATA,
   ...
   public:
    ...
    static constexpr unsigned int getKernelFirRangeLen(int pos) {
        return pos + 1 == fp::BTP_CASC_LEN ? fnFirRangeRemSym(fp::BTP_FIR_LEN, fp::BTP_CASC_LEN, pos)
    ...
    static constexpr unsigned int getFirRangeTapLen(int pos) {
        return CEIL((getKernelFirRangeLen(pos) + 1) / 2, fp::BTP_SSR) / fp::BTP_SSR;
    };

    // Get kernel's FIR range tap vector length
    static constexpr unsigned int getFirRangeTapLen() {
        return CEIL((fp::BTP_FIR_RANGE_LEN + 1) / 2, fp::BTP_SSR) / fp::BTP_SSR;
    };
