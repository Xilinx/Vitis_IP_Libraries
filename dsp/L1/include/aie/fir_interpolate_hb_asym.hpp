...
class kernelFilterClass {
   private:
    ...
    // alignas(__ALIGN_BYTE_SIZE__)  TT_COEFF m_internalTaps[CEIL(m_kNumTaps, m_kCoeffRegVsize)]; // Filter
    // taps/coefficients
    alignas(__ALIGN_BYTE_SIZE__) TT_COEFF m_phaseTwoTap[kMaxColumns] = {nullElem<TT_COEFF>()};
    ...
    static constexpr unsigned int m_kCoeffLoadSize = m_kCoeffRegVsize;

    static constexpr unsigned int internalTapsSize = CEIL(m_kNumTaps + m_kCoeffRegVsize, m_kCoeffRegVsize);
    static constexpr unsigned int InternalCenterTapIndex =
        CEIL(m_kNumTaps, m_kCoeffLoadSize); // separate out center tap from the rest
   ...
   public:
    ...
    static std::vector<TT_COEFF> firReload(const std::vector<TT_COEFF>& taps) {
        ...
        internalTaps[InternalCenterTapIndex] = taps[m_kTotTaps - 1];
        return internalTaps;
    }
...

{
   public:
    ...
    // Constructor
    fir_interpolate_hb_asym(TT_COEFF (&taps)[internalTapsSize]) : internalTaps(taps), thisKernelFilterClass(taps) {}

    // Register Kernel Class
    static void registerKernelClass() {
        REGISTER_PARAMETER(internalTaps);
        REGISTER_FUNCTION(fir_interpolate_hb_asym::filter);
    }
    // FIR
    void filter(
...
class fir_interpolate_hb_asym<TT_DATA,
   ...
   public:
    ...
    static constexpr unsigned int internalTapsSize = thisKernelFilterClass::get_internalTapsSize();
    // Constructor
    fir_interpolate_hb_asym() : thisKernelFilterClass() {}
...
class fir_interpolate_hb_asym_tl : public fir_interpolate_hb_asym<typename fp::BTT_DATA,
   ...
   public:
    ...
    //                        : sr_asym::fnFirRangeRemAsym<fp::BTP_FIR_LEN, fp::BTP_CASC_LEN, pos, typename
    //                        fp::BTT_DATA,
    ...
    static constexpr unsigned int getFirRangeTapLen() {
        return CEIL(getHbTaps(fp::BTP_FIR_RANGE_LEN), fp::BTP_SSR) / fp::BTP_SSR;
    };
